...
function SingleProduct() {
  ...
  const fetchSingleProduct = async () => {
    try {
      const response = await axios.get(`/v1/products/p/${productId}`);
  ...
  const fetchReviews = async () => {
    try {
      const response = await axios.get(`/v1/reviews/get/${productId}`);
      return response.data.data;
    } catch (error) {
      console.log(error.response.data.message || error.response.data);
      throw error.response.data.message || error.response.data;
  ...
  if (productLoading || reviewLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 my-24 bg-gray-100">
        <img
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-4 px-10">
      <div
        onClick={() => navigate(-1)}
      ...
      <div className="w-full p-6 rounded-lg shadow-lg bg-white mt-4">
          ...
          <div className="w-full lg:w-1/2 mx-4 flex flex-col py-6">
            ...
            <div className="text-lg text-gray-700 mb-4">
              ...
              {productData.subCategories?.map((subCategory) => (
                <span
                  key={subCategory._id}
                  className="mr-2 bg-gray-200 px-2 py-1 rounded-md shadow-sm border border-gray-300 text-sm"
                >
            ...
            <button
              ...
              className={`bg-blue-500 text-lg text-white px-4 py-2 rounded-md hover:bg-blue-600 transition duration-300 ease-in-out active:scale-95 md:w-1/4 ${
                productData.quantityInStock > 0 ? "" : "bg-gray-400 cursor-not-allowed"
              }`}
